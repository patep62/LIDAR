...
//  status = VL53L1X_SetInterMeasurementInMs(dev, 200); /* in ms, IM must be > = TB */
  ...
  status = VL53L1X_StartRanging(dev);   /* This function has to be called to enable the ranging */
	...
	for(int i = 0; i < 512; i++) { //For loop allowing for one full rotation of the stepper motor.
//  while(1){  read and display data 
		
		GPIO_PORTL_DATA_R = 0b00001100; 	//The steps required to spin the motor.
		...
		count++; //Increment count to document that the motor has moved 4 steps.
		
		if(count == 4){ //Count will be equal to 4 once the motor has moved through 2.8125 degrees.
			
		//This code then signals the ToF sensor to take one measurement.	
    ...
    debugArray[index] = Distance; //Debugging array.
		index++;
...
//	  status = VL53L1X_GetSpadNb(dev, &SpadNum);
	  ...
	  status = VL53L1X_ClearInterrupt(dev);  //clear interrupt has to be called to enable next interrupt
      sprintf(printf_buffer,"%u\r\n", Distance); //Print only the disance to the UART port.
      UART_printf(printf_buffer);
		count = 0; //Reset count
	...
	//Now I rewind the motor to avoid the wires from getting tangled up.
		for(int i = 0; i < 512; i++) {
